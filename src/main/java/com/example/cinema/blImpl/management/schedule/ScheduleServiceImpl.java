...
public class ScheduleServiceImpl implements ScheduleService, ScheduleServiceForBl {
    ...
    @Override
    public List<ScheduleItem> getScheduleItemByMovieIdAndDate(int movieId, Date date) {
        try {
            return scheduleMapper.selectScheduleByMovieIdAndDate(movieId, date, getNumDayAfterDate(date, 1));
        }catch (Exception e){
            e.printStackTrace();
            return null;
        }
