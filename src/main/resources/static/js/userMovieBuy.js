...
$(document).ready(function () {
    ...
    function getInfo() {
        getRequest(
            ...
            function (res) {
                if (res.success) {
                    schedule=res.content.scheduleItem;
...
function orderConfirmClick() {
    ...
    var _userId=sessionStorage.getItem('id')
    if(!scheduleId){
        scheduleId = parseInt(window.location.href.split('?')[1].split('&')[1].split('=')[1]);
    }
    var seats=[]
    selectedSeats.forEach(element => {
        var seatForm={}
        seatForm.columnIndex=element[1]
        seatForm.rowIndex=element[0]
        seats.push(seatForm)
    });
    var formData={
        userId:_userId,
...
function postPayRequest() {
    //需将ticketId数组处理为list，否则后端无法识别 xzt0427
    ...
    url=url+"?couponId="+order.couponId;
    for(var i=0;i<order.ticketId.length;i++){
        url+="&ticketId="+order.ticketId[i]
    }
    postRequest(
        url,
        null,
        function (res) {
            if (res.success) {
                ...
                console.log("购票失败"+JSON.stringify(res));
            }
        },
        function (error) {
            alert("购票出错"+JSON.stringify(error));
        }   
    )
}
